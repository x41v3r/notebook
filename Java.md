...
* 标识符命名规范（推荐但不强制）：
	...
	* 变量（属性）名、方法名：多单词组成时，除第一个单词之外每一个单词的首字母都大写； 常量名：每一个字母都大写，多单词时每个单词之间用`_`连接。
* 注意：一般情况下，**Java 项目中类的包名不能以 java.开头**，否则执行时就会抛出异常。

#### （3）Java中的变量

> 变量：其实就是内存中的一段存储区域，能够在同一个范围内不断变化，是程序的最基本的存储单元

* Java 中的变量必须在声明之后使用，因为只有声明后这个变量才会在内存中被加载
...
* `char c1 = ‘a’;`
* 注意：
	* java 采用 unicode 编码，支持汉字、单位、数字等各种字符
...
* `boolean`
* boolean 类型的变量只能取两个值之一：**true** 或 **false**
* boolean 类型在内存中所占的存储空间大小并没有明确的规定
...
* 容量（表示范围）从小到大排序：**（byte 、char、 short） -> int -> long -> float -> double**
* 注意事项
	* 此处的容量大小是指**能够表示的数值范围的大小**，而不是占用的内存空间的大小
	* 当 byte、char、short 三个类型的变量之间进行运算时，**结果的类型都一定会是 int**，这三种类型各自同类型之间进行运算（即 byte 和 byte、char 和 char、short 和 short）时也是这样

> 强制数据类型转换
* double doubleVar = 12.9;
* int intVar = (int)doubleVar;
* 注意事项
	* 浮点型向整形强制类型转换，采用的是**直接截取浮点数整数部分的方式（俗称“截断操作”）**
	    * 而不是四舍五入，转换之后数据的精度可能会降低，称为精度损失
...
* String 类型值的**双引号内可以有一个或多个字符，也可以什么都没有**
* String 可以分别和 8 种基本的数据类型进行运算，但是都只能是字符串的拼接运算
...
<!--![Java运算符的优先级和结合性](./pictures/Java运算符的优先级和结合性.png)-->
...
* 注意事项：
	* else结构是可选的
	* 如果多个条件表达式之间是 “互斥的关系”（或没有交集的关系），那个判断或执行语句声明在上面还是下面，是无所谓的（感觉这句话也没什么用）
...
* 如果想要实现多选一的操作，那么应该在每一个case中都使用`break;`语句
* 表达式的值**只能是这6种类型之一**：byte、short、char、int、枚举类型、String类型
  * JDK5.0开始枚举类型才允许作为表达式
  * JDK7.0开始String类型才允许作为表达式
* case关键字后面绝对**禁止写范围**
...
* 这4种权限修饰符都可以用来修饰类的这几个内部结构（其实就是不包括代码块）：属性、方法、构造器和内部类
...
* 注意事项
  ...
  * Java中的任何一个类，不管它被什么修饰，内部都**至少有一个构造器**（或显式的或隐式的）
...
* Java中关于继承性的规定
  ...
  * 子类继承父类以后，就获取了直接父类以及所有间接父类中声明过的属性和方法
* Object类
...
* 方法重写的规定（约定：子类中的叫 “重写的方法”，父类中的叫 “被重写的方法”）
  ...
  * 关于返回值类型
    ...
    * 基本数据类型：父类被重写方法的返回值类型是基本数据类型（如：double），则子类重写的方法的返回值类型**必须是相同的基本数据类型**（必须也是double，绝对不能是int、float或者其他的什么类型）
...
* 我们可以在子类或构造器中，通过使用`super.属性`或`super.方法`的方式，显式的调用父类中声明的属性或方法。通常情况下，我们都习惯省略`super.`
  * 当子类和父类中定义了同名属性时，我们要想在子类中调用父类中声明的属性，则必须显式地使用`super.属性`的方式，表明调用的是父类中声明的属性
...
> 对象的多态性：父类的引用指向子类的对象  
> 通过多态性创建的对象，在编译时被认为是父类的对象，不能调用子类特有的方法、属性
...
* static关键字可以用来修饰：属性、方法、代码块和内部类

#### （1）使用static修饰属性

* 类的属性按照是否使用static修饰，分为静态变量（类变量）和非静态变量（实例变量）
  * 实例变量：我们创建了类的多个对象，每个对象都独立的拥有一套类的非static属性。当修改其中任何一个对象中的非静态属性时，不会导致其他对象中同样的属性的值被修改
  * 静态变量：我们创建了类的多个对象，多个对象共享同一个静态变量。当通过对象修改一个静态变量时，会导致其他对象调用此变量的值是修改过了的

* 关于static属性的其他说明
  * 静态变量随着类的加载而加载，可以通过`类名.静态变量变量名`的方式直接进行调用
  * 静态变量的加载要早于对象的创建（实例变量随着对象的加载而加载，在对象被创建之后才会被加载）
  * 由于类只会被加载一次，则静态变量在内存中也只会存在一份（存在方法区的静态域中）

#### （2）使用static修饰方法

* 随着类的加载而加载、可以通过`类名.静态方法名()`的方式进行调用
* 通过类不能直接调用非static的方法
* 静态方法和非静态方法
  * 静态方法中只能嗲用静态的方法或属性，因为他们的生命周期是一致的
  * 非静态方法中既可以调用静态的方法和属性又可以调用静态的方法和属性
* 在static方法中，不能使用this和super关键字

#### （3）在开发中如何确定一个属性是否声明为static的？

* 属性是可以被多个对象所共享的，不会随着对象的不同而不同
* 类中的常量也常常声明为static的

#### （4）在开发中如何确定一个方法是否声明为static的？
...
* 代码块的作用：初始化当前的类和对象
* 代码块如果有修饰的话，只能用static来修饰

  * 静态代码块：随着类的加载而执行（只执行一次，除非类被重新加载）
    * 作用：初始化类的信息（静态的属性）
    * 如果一个类中定义了多个静态代码块，则按照声明的先后顺序执行，但是一般不会声明多个静态代码块
    * 可以调用同类中静态的结构，不可以调用同类中的非静态结构

  * 非静态代码块：随着对象的创建而执行（每创建一个对象就会执行一次）
    * 作用：可以在创建对象时，对对象的属性的进行初始化
    * 如果一个类中定义了多个非静态代码块，则也会按照声明的先后顺序执行，但是也不会声明多个非静态代码块
    * 可以调用同类中的静态或非静态结构
* 代码块是不能被主动调用的，只能自动执行
* 静态代码块的执行会先于非静态代码块的执行
* 代码块的一个重要作用是，我们要为类的属性进行初始化，但实际初始化操作不是简单的一个数值，而是需要抛出异常的一段代码，那么最好使用代码块来对类的属性进行初始化
* 在创建类的对象时，会**先执行非静态代码块的代码，后执行所调用的构造器中的代码**
...
* 修饰方法：被修饰的方法不可以再被重写
  * 举例：Object类中的`getClass();`方法

```java
public final void theMethod(){
    
}
```

* 修饰属性：final修饰的属性不能再使用默认初始化，不能使用方法初始化
  * 显式初始化
  * 在代码块中初始化
  * 在构造器中初始化（要求每个构造器内都要写初始化代码，这是为了确保final属性的初始化一定能被完成）
...
* 修饰局部变量
  * final修饰方法的形参：传参之后，参数变量的值在这个方法体内不能被修改，只能调用
  * final修饰方法体内的变量：这个变量就是一个常量，这个变量的值不可更改
...
抽象类：随着继承层次中一个个新子类的定义，类变得越来越具体，而父类则更一般，更通用。类的设计应该保证父类和子类能够共享特征。有时候将一个父类设计的非常抽象，以至于它没有具体的实例，这样的类就叫做抽象类。

* abstract修饰的类
  * 无法被实例化，不能创建对象，只用于继承
  * 抽象类不能创建对象，但是仍然存在构造器，用于被子类实例化时调用
  * 开发中都会i提供抽象类的子类，让子类实例化，完成相关的操作
...
* abstract修饰方法
  * 只有方法的声明，没有方法体
  * 我们必须保证一个抽象方法所在的类不能实例化，即抽象方法只能声明在抽象类之中（包含抽象方法的类一定是一个抽象类）
  * 若子类重写了父类中的所有抽象方法，则此类方可实例化；若子类没有重写父类中的所有抽象方法，则子类也必须是一个抽象类，需要用abstract修饰

```java
abstract public class TestClass{
    public abstract void testAbstractOne(int a);//这是一个抽象的方法
    public abstract void testAbstractTwo(String b);//这是一个抽象的方法
}

class SubClassOne extends TestClass{
    public void testAbstractOne(int a){
        System.out.println("方法一");
    }
    public void testAbstractTwo(String b){
        System.out.println("方法二");
    }
}//两个抽象方法，都进行了重写，本类可以实例化

abstract class SubclassTWO extends TestClass{
    public void testAbstractTwo(String b){
        System.out.println("方法二");
    }
}//只重写了抽象方法testAbstractTwo，没有重写抽象方法testAbstractOne()，类必须是abstract的
```

* 应用场景：在父类中的某个方法，在不同的具体子类中的的实现是不同的，完全无法确定该如何编写父类中的这个方法
* 注意事项
  * abstract**不能用来修饰属性和构造器等结构**
  * abstract不能修饰private方法、static方法、final的方法和final的类
  * 利用多态性可以使得抽象类的引用指向他的某个子类的对象实例
...
## 6 JavaWeb应用开发（Servlet+JSP+JDBC）
