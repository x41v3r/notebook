...
* 如果想要**通过整数之间的除法得到精确的结果**，那么就需要对参加运算的整数进行手动的类型提升
...
<!-- &emsp;&emsp;Java引用变量有两个类型：**编译时类型**和**运行时类型**。  
&emsp;&emsp;编译时类型由声明该变量的类型决定，运行时类型由实际赋给该变量的对象决定。简称为：编译时看左边，运行时看右边。
